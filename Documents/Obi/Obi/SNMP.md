...
```shell-session
NivaLada@htb[/htb]$ cat /etc/snmp/snmpd.conf | grep -v "#" | sed -r '/^\s*$/d'
...
```shell-session
NivaLada@htb[/htb]$ snmpwalk -v2c -c public 10.129.14.128
...
```shell-session
NivaLada@htb[/htb]$ sudo apt install onesixtyone
NivaLada@htb[/htb]$ onesixtyone -c /opt/useful/SecLists/Discovery/SNMP/snmp.txt 10.129.14.128
...
```shell-session
NivaLada@htb[/htb]$ sudo apt install braa
NivaLada@htb[/htb]$ braa <community string>@<IP>:.1.3.6.*   # Syntax
NivaLada@htb[/htb]$ braa public@10.129.14.128:.1.3.6.*
